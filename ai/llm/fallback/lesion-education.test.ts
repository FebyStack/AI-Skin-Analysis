...
function analysis(top: { label: string; confidence: number }[]): LesionAnalysis {
  return {
    lesions: [{ bbox: null, detectorConfidence: null, localizationConfidence: 0.2, segmented: false, classification: { predicted: top[0]?.label ?? null, confidence: top[0]?.confidence ?? 0, top } }],
